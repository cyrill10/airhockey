...
var brickWidth = 10 * gameScale;
var brickHeight = 10 * gameScale;
var brickPadding = 4 * gameScale;
var brickOffsetTop = 15 * gameScale;
var brickOffsetLeft = 2 * gameScale;
var brickRowCount = Math.floor(
  canvas.width / (brickWidth + brickPadding + brickOffsetLeft)
);
...
function wait() {
  ...
  overlay.setBanner(config.general.name);
  overlay.setButton(config.general.buttonText);
  ...
  for (var c = 0; c < brickColumnCount; c++) {
    for (var r = 0; r < brickRowCount; r++) {
      if (bricks[c][r].status == 1) {
        var brickX =
          r * (brickWidth + brickPadding) + brickOffsetLeft + rowOffset;
